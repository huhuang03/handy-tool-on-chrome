...
const openType: OpenType = OpenType.CLICK
...
function openAll() {
  ...
  if (openType == OpenType.BACKGROUND) {
...
async function init() {
  ...
  const playX5AllButton = createButton(openAllSvgPathList, `全部x${config.bilibiliOpenAllPages}`, () => {
    repeatOpenAll(config.bilibiliOpenAllPages)
  })
